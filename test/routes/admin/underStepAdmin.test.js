...
describe("Admin tests", () => {
    ...
    describe("[INVALID ADMIN UNDER STEPS TESTS]", () => {
        ...
        test("[GET ALL] missing language : should not get the steps with a 400 status code", async() => {
            const response = await request(server).get("/admin/underStep/getAll?stocked_title=test&step_id=1");
            expect(response.statusCode).toBe(400);
        });
        test("[GET ALL] missing step_id : should not get the steps with a 400 status code", async() => {
            const response = await request(server).get("/admin/underStep/getAll?stocked_title=test&language=english");
            expect(response.statusCode).toBe(400);
        });
        ...
        test("[GET ALL] empty all : should not get the steps with a 400 status code", async() => {
            const response = await request(server).get("/admin/underStep/getAll?stocked_title=&language=&step_id=");
            expect(response.statusCode).toBe(400);
        });
        test("[GET ALL] empty stocked_title : should not get the steps with a 400 status code", async() => {
            const response = await request(server).get("/admin/underStep/getAll?language=english&step_id=1&stocked_title=");
            expect(response.statusCode).toBe(400);
        });
        test("[GET ALL] empty language : should not get the steps with a 400 status code", async() => {
            const response = await request(server).get("/admin/underStep/getAll?stocked_title=test&step_id=1&language=");
            expect(response.statusCode).toBe(400);
        });
        ...
        test("[ADD] missing stocked_title : should not add the step with a 400 status code", async() => {
            const response = await request(server).post(`/admin/underStep/add?newUnderStep=${newUnderStep}&step_id=1`);
            expect(response.statusCode).toBe(400);
        });
        test("[ADD] missing step_id : should not add the step with a 400 status code", async() => {
            const response = await request(server).post(`/admin/underStep/add?newUnderStep=${newUnderStep}&stocked_title=test`);
            expect(response.statusCode).toBe(400);
        });
        test("[ADD] missing newUnderStep : should not add the step with a 400 status code", async() => {
            const response = await request(server).post(`/admin/underStep/add?step_id=1&stocked_title=test`);
            expect(response.statusCode).toBe(400);
        });
        test("[ADD] missing all : should not add the step with a 400 status code", async() => {
            const response = await request(server).post(`/admin/underStep/add`);
            expect(response.statusCode).toBe(400);
        });
        test("[ADD] empty all : should not add the step with a 400 status code", async() => {
            const response = await request(server).post(`/admin/underStep/add?newUnderStep=&step_id=&stocked_title=`);
            expect(response.statusCode).toBe(400);
        });
        test("[ADD] empty stocked_title : should not add the step with a 400 status code", async() => {
            const response = await request(server).post(`/admin/underStep/add?newUnderStep=${newUnderStep}&step_id=1&stocked_title=`);
            expect(response.statusCode).toBe(400);
        });
        test("[ADD] empty step_id : should not add the step with a 400 status code", async() => {
            const response = await request(server).post(`/admin/underStep/add?newUnderStep=${newUnderStep}&step_id=&stocked_title=test`);
            expect(response.statusCode).toBe(400);
        });
        ...
        test("[MODIFY] missing stocked_title : should not modify the step with a 400 status code", async() => {
            const response = await request(server).post(`/admin/underStep/modify`).send({ newUnderStep: newUnderStep, step_id: 1, underStep_id: 1, language: "english" });
            expect(response.statusCode).toBe(400);
        });
        test("[MODIFY] missing step_id : should not modify the step with a 400 status code", async() => {
            const response = await request(server).post(`/admin/underStep/modify`).send({ newUnderStep: newUnderStep, stocked_title: "test", underStep_id: 1, language: "english" });
            expect(response.statusCode).toBe(400);
        });
        test("[MODIFY] missing newUnderStep : should not modify the step with a 400 status code", async() => {
            const response = await request(server).post(`/admin/underStep/modify`).send({ step_id: 1, stocked_title: "test", underStep_id: 1, language: "english" });
            expect(response.statusCode).toBe(400);
        });
        test("[MODIFY] missing underStep_id : should not modify the step with a 400 status code", async() => {
            const response = await request(server).post(`/admin/underStep/modify`).send({ newUnderStep: newUnderStep, step_id: 1, stocked_title: "test", language: "english" });
            expect(response.statusCode).toBe(400);
        });
        test("[MODIFY] missing language : should not modify the step with a 400 status code", async() => {
            const response = await request(server).post(`/admin/underStep/modify`).send({ newUnderStep: newUnderStep, step_id: 1, stocked_title: "test", underStep_id: 1 });
            expect(response.statusCode).toBe(400);
        });
        ...
        test("[MODIFY] empty all : should not modify the step with a 400 status code", async() => {
            const response = await request(server).post(`/admin/underStep/modify`).send({ newUnderStep: "", step_id: "", stocked_title: "", underStep_id: "", language: "" });
            expect(response.statusCode).toBe(400);
        });
        test("[MODIFY] empty stocked_title : should not modify the step with a 400 status code", async() => {
            const response = await request(server).post(`/admin/underStep/modify`).send({ newUnderStep: newUnderStep, step_id: 1, stocked_title: "", underStep_id: 1, language: "english" });
            expect(response.statusCode).toBe(400);
        });
        test("[MODIFY] empty step_id : should not modify the step with a 400 status code", async() => {
            const response = await request(server).post("/admin/underStep/modify").send({ newUnderStep: newUnderStep, step_id: "", stocked_title: "test", underStep_id: 1, language: "english" });
            expect(response.statusCode).toBe(400);
        });
        test("[MODIFY] empty newUnderStep : should not modify the step with a 400 status code", async() => {
            const response = await request(server).post("/admin/underStep/modify").send({ newUnderStep: "", step_id: 1, stocked_title: "test", underStep_id: 1, language: "english" });
            expect(response.statusCode).toBe(400);
        });
        test("[MODIFY] empty underStep_id : should not modify the step with a 400 status code", async() => {
            const response = await request(server).post("/admin/underStep/modify").send({ newUnderStep: newUnderStep, step_id: 1, stocked_title: "test", underStep_id: null, language: "english" });
            expect(response.statusCode).toBe(400);
        });
        test("[MODIFY] empty language : should not modify the step with a 400 status code", async() => {
            const response = await request(server).post("/admin/underStep/modify").send({ newUnderStep: newUnderStep, step_id: 1, stocked_title: "test", underStep_id: 1, language: "" });
            expect(response.statusCode).toBe(400);
        });
