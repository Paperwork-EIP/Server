...
describe("User Under Step Persistence Tests", () => {
    ...
    it('[GET ALL BY STEP ID] should get all under steps by step id', async() => {
        const result = {
            id: 1,
            step_id: step_id
        };
        ...
        db.query = jest.fn().mockReturnValue({ rows: [result] });
        const response = await UserUnderStep.getAllByStepId(step_id);

        expect(response).toEqual(expect.any(Object));
        expect(response).toEqual(expectedResponse);
        expect(db.query).toHaveBeenCalled();
    });
    it('[GET ALL NOT DONE BY STEP ID] should get all under steps by step id', async() => {
        const result = {
            id: 1,
            step_id: step_id
        };
        ...
        db.query = jest.fn().mockReturnValue({ rows: [result] });
        const response = await UserUnderStep.getAllNotDoneByStepId(step_id);
