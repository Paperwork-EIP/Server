...
describe("Step Persistence Tests", () => {
    ...
    it('[UPDATE] should update the step with the given id', async() => {
        const id = 7;
        const is_unique = false;
        const delay = "0010";

        const response = await Step.update(id, delay, is_unique);

        expect(response).not.toBeNull();
    });
    it('[UPDATE] should throw an error if an error occurs', async() => {
        jest.spyOn(db, 'query').mockRejectedValueOnce(() => { throw new Error });
        await expect(Step.update()).rejects.toThrow();
    });
    it('[DELETE] should delete the step with the given id', async() => {
        const id = 7;
        const response = await Step.delete(id);

        expect(response).not.toBeNull();
    });
    it('[DELETE] should throw an error if an error occurs', async() => {
        jest.spyOn(db, 'query').mockRejectedValueOnce(() => { throw new Error });
        await expect(Step.delete()).rejects.toThrow();
    });
});